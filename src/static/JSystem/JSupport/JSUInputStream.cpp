...
int JSURandomInputStream::skip(s32 amount) {
    int s = this->seekPos(amount, (JSUStreamSeekFrom)1);
    if (s != amount) {
        this->setState((EIoState)1);
    }
    return s;
...
int JSURandomInputStream::align(s32 alignment) {
    ...
    if (change != 0) {
        int s = this->seekPos(aligned, (JSUStreamSeekFrom)0);
        if (s != change) {
            this->setState((EIoState)1);
        }
    }
...
int JSURandomInputStream::peek(void* buf, s32 len) {
    ...
    if (r != 0) {
        this->seekPos(pos, (JSUStreamSeekFrom)0);
    }
